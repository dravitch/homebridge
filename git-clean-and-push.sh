...
# Supprimer les backups Windows (*.backup.*)
find . -path "./.git" -prune -o -name "*.backup.*" -type f -print -delete 2>/dev/null || true

# Supprimer les fichiers temporaires
find . -path "./.git" -prune -o -name "*.tmp" -type f -print -delete 2>/dev/null || true

# Supprimer les patches (mais PAS git-clean-and-push.sh ni git-sync.sh)
find . -path "./.git" -prune -o -name "*.patch" -type f -print -delete 2>/dev/null || true

# Supprimer les logs
find . -path "./.git" -prune -o -name "*.log" -type f -print -delete 2>/dev/null || true

# Supprimer les anciens scripts obsolètes (pas les nouveaux)
[[ -f gitinit.sh ]] && rm -f gitinit.sh && echo "  Supprimé: gitinit.sh (obsolète)"
[[ -f reset_publish.sh ]] && rm -f reset_publish.sh && echo "  Supprimé: reset_publish.sh (obsolète)"
[[ -f git_clean.sh ]] && rm -f git_clean.sh && echo "  Supprimé: git_clean.sh (obsolète)"
...
# Vérifier qu'on ne commit pas les scripts essentiels par erreur
if ! git diff --cached --name-only | grep -q "git-clean-and-push.sh"; then
    echo "  ⚠️  git-clean-and-push.sh n'est pas dans le commit"
    echo "  Ajout forcé..."
    git add -f git-clean-and-push.sh
fi

if ! git diff --cached --name-only | grep -q "git-sync.sh"; then
    echo "  ⚠️  git-sync.sh n'est pas dans le commit"
    echo "  Ajout forcé..."
    git add -f git-sync.sh
fi
...
- Cleaned: Removed temporary files and obsolete scripts
- Support: Multi-machine configuration ready

Workflow:
- just sync: Daily commit + push
- just reset: Clean reset (this script)
- just check: Verify no secrets
